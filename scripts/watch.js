...
const setupSharedProcessPackageWatcher = ({ ws }) =>
  getWatcher({
    name: 'reload-page-on-sharedProcess-package-change',
    configFile: 'packages/sharedProcess/vite.config.js',
    writeBundle() {
      ws.send({
        type: 'full-reload',
      });
...
(async () => {
  try {
    ...
    await setupSharedProcessPackageWatcher(viteDevServer);
