...
class ValidatedIpcMain implements Event.NodeEventEmitter {
  ...
  private validateEvent(channel: string, event: IpcMainEvent | IpcMainInvokeEvent): boolean {
    if (!channel || !channel.startsWith('vscode:')) {
      console.log(`Refused to handle ipcMain event for channel '${channel}' because the channel is unknown.`);
    ...
    try {
      ...
      console.log(`Refused to handle ipcMain event for channel '${channel}' because of a malformed URL '${url}'.`);
      return false; // unexpected URL
    }

    // if (host !== 'vscode-app') {
    //   console.log(`Refused to handle ipcMain event for channel '${channel}' because of a bad origin of '${host}'.`);
    //   return false; // unexpected sender
    // }

    if (sender.parent !== null) {
      console.log(`Refused to handle ipcMain event for channel '${channel}' because sender of origin '${host}' is not a main frame.`);
